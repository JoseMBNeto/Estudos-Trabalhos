...
void enfileirar (Fila *f, int id, const char *nome, bool ehfan, int extra){
    ...
    int pessoasAfrente = f->tamanho + extra;
    ...
    while (opcao != 0) {
        ...
        switch (opcao) {
            case 1: {
                ...
                if (respFan == 1){
                    enfileirar(filaFa, proximoId++, nome, respFan == 1, 0);
                }else{
                    enfileirar(filaVirtual, proximoId++, nome, respFan == 0, filaFa->tamanho);
                }
                break;
            }

            case 2: {
                if (filaVirtual->inicio == NULL && filaFa->inicio == NULL){
                ...
                if (filaFa->inicio != NULL){
                    atenderProximo(filaFa, &bilheteria, tipoIngresso);
                }else{
